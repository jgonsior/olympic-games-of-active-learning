...
for auc_prefix in [
    ...
    # for dataset in ts["EXP_DATASET"].unique():
    ts = ts_orig
    # ts = ts.loc[ts["EXP_DATASET"] == dataset]

    for target_to_evaluate in targets_to_evaluate:
        correlation_data_path = Path(
            config.OUTPUT_PATH
            / f"plots/single_hyperparameter/{target_to_evaluate}/single_indice_{target_to_evaluate}_{auc_prefix}_{standard_metric}_statistic.parquet"
        )
        ...
        match auc_prefix:
            case "full_auc_":
                ts["metric_value"] = ts["metric_value"].parallel_apply(lambda lll: lll)
            case "first_5_":
        ...
        for rank_measure in ["statistic", "pvalue", "jaccard"]:
            ...
            sums = jaccards2.abs().sum() / len(jaccards2)
            ...
            save_correlation_plot(
                data=corrmat,
                title=f"single_hyperparameter/{target_to_evaluate}/single_indice_{target_to_evaluate}_{auc_prefix}_{standard_metric}_{rank_measure}",
                keys=keys,
                config=config,
            )

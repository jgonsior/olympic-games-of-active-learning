...
Most evaluation scripts require time series data in `_TS/`. If missing, run this first:
...
|--------|-------|----------|-------------|
| `learning_curve.py` | Per-cycle CSVs, `05_done_workload.csv` | `_TS/*.parquet` | **Run first.** Builds sorted time series parquets from raw per-cycle CSVs. Required by most other scripts. |
...
| `scripts/reduce_to_dense.py` | Filter all files to keep only experiments in the dense workload. |
...
| Issue | Solution |
|-------|----------|
| Missing `_TS/*.parquet` | Run `eva_scripts/learning_curve.py` first |
| Memory errors during evaluation | Use `scripts/reduce_to_dense.py` for a smaller subset |

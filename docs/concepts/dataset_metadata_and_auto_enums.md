...
Downstream usage:
- eva scripts can join categorizations with `_TS/*.parquet` time-series (via `misc/helpers.py::create_fingerprint_joined_timeseries_csv_files`) to correlate dataset traits with strategy performance.
